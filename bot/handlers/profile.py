...
async def change_profile_parameter(callback_query: CallbackQuery, state: FSMContext):
    ...
    action = callback_query.data.split("_")[1]  # Получаем параметр (name, age, height, weight, goal)
...
# Обработчик для получения нового значения роста
...
# Обработчик для получения нового значения веса
