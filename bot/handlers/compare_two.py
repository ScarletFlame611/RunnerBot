...
def calculate_average_speed(distance, duration):
    ...
    # Средняя скорость: дистанция / время (в минутах)
...
async def compare_trainings_command(message: Message):
    ...
    distance_comparison = "Больше" if distance_1 > distance_2 else ("Меньше" if distance_1 < distance_2 else "Равны")
    duration_comparison = "Больше" if duration_1 > duration_2 else ("Меньше" if duration_1 < duration_2 else "Равны")
    speed_comparison = "Больше" if speed_1 > speed_2 else ("Меньше" if speed_1 < speed_2 else "Равны")
    ...
    # Отправляем сообщение с результатами сравнения
